...
class CNN:
    ...
    def evaluate(self, data: Dict[str, Any],dataset_name,type_problem, batch_size: int = 64) -> Tuple[float, float]:
        ...
        if self.dataset_n=='chestdata' or self.dataset_n=='brain' or self.dataset_n=='breast':
          for i in range(0,2000):
                if data['y_test'][random.randint(0,len(data['x_test']))]==1:
                  img=data['x_test'][random.randint(0,len(data['x_test']))]
                  break
                else:
                  print("running")
                  continue
