...
class AutoCNN:
    ...
    def get_output_function(self) -> Callable[[tf.keras.layers.Layer], tf.keras.layers.Layer]:
        ...
        def output_function(inputs):
            ...
            if self.type_problem=='multi-label binary-class':
              ...
              return tf.keras.layers.Dense(output_size, activation='sigmoid')(out)
    ...
    def __init__(self, population_size: int,
        ...
        self.dataset_n=dataset_name
        self.dataset_name = dataset_name+str(random.randint(4,1000))+'.csv'
        self.fitness_file= dataset_name+str(random.randint(10,1000))+'.txt'
        lines = ['Fitness']
        ...
        f = open(self.dataset_name, 'w')
        # create the csv writer
        writer = csv.writer(f)
        # write a row to the csv file
        row=['Accuracy','Precision','Recall','F1-Score','AUC','CNN','Params','Layers','Time']
        writer.writerow(row)
        # close the file
        f.close()
    ...
    def initialize(self) -> None:
        ...
        for _ in range(self.population_size):
            ...
            print("Layers appended {}".format(layers))
    ...
    def evaluate_individual_fitness(self, cnn: CNN) -> None:
        ...
        """
        try:
            cnn.generate(self.dataset)

            cnn.train(self.dataset, epochs=self.epoch_number)
            loss,accuracy = cnn.evaluate(self.dataset,self.dataset_name,self.type_problem)
            params= cnn.model.count_params()
            ...
            accuracy = 0
        print("fitness")
        self.fitness[cnn.hash] = 0.2*(100*accuracy)+0.2*(100*cnn.F1_Score)-0.2*math.log(params)
        with open(self.fitness_file, 'a') as f:
          f.write(str(self.fitness[cnn.hash]))
          f.write('\n')
        f.close()
    ...
    def split_individual(self, cnn: CNN) -> Tuple[Sequence[Layer], Sequence[Layer], Sequence[Layer]]:
        """
        Randomly splits the CNN layers into four
        :param cnn: the CNN to split the layers
        :return: the four parts of the CNN layers
        """
        n=4
        k, m = divmod(len(cnn.layers), 4)
        return (cnn.layers[i*k+min(i, m):(i+1)*k+min(i+1, m)] for i in range(n))
    ...
    def generate_offsprings(self) -> List[CNN]:
        ...
        while len(offsprings) < len(self.population):
            ...
            #p3 = self.select_two_individuals(self.population)
            ...
            if r < self.crossover_probability:
                p1_1, p1_2, p1_3,p1_4 = self.split_individual(p1)
                p2_1, p2_2, p2_3,p2_4 = self.split_individual(p2)


                o1 = [*p1_1, *p2_2,*p1_3,*p2_4]
                o2 = [*p2_1, *p1_2,*p2_2,*p1_4]
    ...
    def generate_cnn(self, layers: Sequence[Layer]) -> CNN:
        ...
        return CNN(self.input_shape,self.type_problem,self.dataset_n, self.output_layer, layers, optimizer=self.optimizer, loss=self.loss,
